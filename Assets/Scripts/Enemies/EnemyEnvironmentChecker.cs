...
namespace Enemies
{
    public class EnemyEnvironmentChecker : EnvironmentChecker
    {
        ...
        public EnemyEnvironmentChecker(LayerMask walkLayers, Transform enemy, Transform player, Transform feetPosition, Vector2 sizeCapsule, float angleCapsule, float delayJumpTime) : base(feetPosition, sizeCapsule, walkLayers, angleCapsule, delayJumpTime)
        {
        ...
        public bool IsCliffAhead(int direction)
        {
            ...
            {
                var checkPosition = new Vector2(_enemy.position.x + direction * GroundCheckDistance + xOffset, _enemy.position.y);
        ...
        {
            return Mathf.Abs(_player.position.x - _enemy.position.x) < visionDistance && Mathf.Abs(_player.position.y - _enemy.position.y) < visionDistance;
        }
    }
}